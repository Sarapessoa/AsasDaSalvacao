...
public class Plane : MonoBehaviour {
    ...
    public float TimerToNextMissile { get; private set; }
    ...
    void UpdateWeaponCooldown(float dt) {
        ...
        if(MissileCount > 0) TimerToNextMissile = 0;
            ...
            float timerToNextMissile = 0f;
            for(int i = 0; i < missileReloadTimers.Count; i++){
                if(timerToNextMissile <= 0) {
                    timerToNextMissile = missileReloadTimers[i];
                    continue;
                };
                timerToNextMissile = Mathf.Min(missileReloadTimers[i], timerToNextMissile - dt);
            }
            TimerToNextMissile = timerToNextMissile;
        }
    }
    ...
    void MaintainConstantSpeed() {
        float desiredSpeed = 200f;
        Vector3 currentVelocity = Rigidbody.velocity;
        float currentSpeed = currentVelocity.magnitude;

        if (Mathf.Abs(currentSpeed - desiredSpeed) > 0.1f) {
            float speedDifference = desiredSpeed - currentSpeed;
            Vector3 adjustment = currentVelocity.normalized * speedDifference;
            Rigidbody.AddForce(adjustment, ForceMode.VelocityChange);
        }
    }
    ...
    void OnCollisionEnter(Collision collision) {
        ...
        for (int i = 0; i < collision.contactCount; i++) {
            ...
            collisionRotation = Quaternion.Euler(0, Rigidbody.rotation.eulerAngles.y, 0);
        }

        if (validCollision) {
            Health = 0;
            Rigidbody.isKinematic = true;
            Rigidbody.position = collisionPoint;
            Rigidbody.rotation = collisionRotation;
