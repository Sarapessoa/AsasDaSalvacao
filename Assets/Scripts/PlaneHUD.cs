...
public class PlaneHUD : MonoBehaviour {
    ...
    void UpdateMissile(){
        missileUI.SetMissileCount(plane.MissileCount);
        missileUI.SetTimerToNextMissile(plane.TimerToNextMissile);
        missileText.text = string.Format("{0:0}", plane.MissileCount); 
    }
